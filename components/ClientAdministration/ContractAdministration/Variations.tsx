...
// export default function Variations({ clientID } : PageProps) {
...
import { getVariations } from "@/services/clientAdministration/contractAdministrationServices";

import Variations_Data from "./Variations_Data";
...
export default function Variations({ clientID } : PageProps) {
  ...
  const [ variations, setVariations ] = useState<VariationProps[]>([]);
  
  useEffect(() => {
    getVariations(clientID)
      .then(data => {
        setVariations(data)
      })
      .catch(error => {
  ...
  const refreshList = async () => {
    const data = await getVariations(clientID);
  ...
  return (
    <>
        ...
        <table className="w-full">
          ...
          <tbody className="text-portalText py-2">
            {variations.map((item,index) => (
              <Variations_Data document={item} clientID={clientID} refreshList={refreshList} key={index}/>
            ))}
          </tbody>
        </table>
      </div>
      ...
      <Add_Variation isOpen={openAddModal} closeModal={() => setOpenAddModal(false)} clientID={clientID} refreshList={refreshList}/>
    </>
  )
}
