...
import { updateClient } from '@/services/administration/clientServices';
...
export default function Edit_Client({ isOpen, closeModal, clientData, refreshClientList } : ClientProps) {
  ...
  const [ selectValue, setSelectValue ] = useState<object | undefined>({ value: "", label: ""});
  ...
  useEffect( () => {
    ...
    const defaultValue = ClientTypeOptions.find(option => option.value === clientData.type);
          ...
          <div className='flex min-h-full items-center justify-center text-center top-[-100vw] static w-full h-full'>
            ...
            >
              <Dialog.Panel className='relative w-[525px] h-auto overflow-y-auto transform rounded-[20px] bg-white text-left shadow-xl transition-all text-dark border border-[#7D7D7D] p-[30px]'>
                <div className={`absolute top-0 left-0 w-full h-full bg-[#ffffff96] justify-center items-center text-tertiary text-[70px] z-[21] ${loading ? "flex" : "hidden"}`}><FaSpinner className="animate-spin"/></div>
                <h3 className="text-[#1C7FCD] text-[25px] font-[800] mb-[25px]">Edit Client</h3>
