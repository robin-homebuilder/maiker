...
import { updateClientDocument } from '@/services/clientAdministration/clientInformationServices';
...
interface ClientProps {
  isOpen: boolean;
  closeModal: () => void;
  clientDocument: ClientDocumentsProps;
  clientID: string;
  refreshList: () => void;
  showDeleteModal: () => void;
}

export default function Edit_ClientInformation_Document({ isOpen, closeModal, clientDocument, clientID, refreshList, showDeleteModal } : ClientProps) {
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    await updateClientDocument({ data: formData, clientID: clientID});
          ...
          <div className='flex min-h-full items-center justify-center text-center top-[-100vw] static w-full h-full'>
            ...
            >
              <Dialog.Panel className='relative w-[525px] h-auto transform rounded-[20px] bg-white text-left shadow-xl transition-all text-dark border border-[#7D7D7D] p-[30px]'>
                ...
                <h3 className="text-[#1C7FCD] text-[25px] font-[800] mb-[25px]">Edit Client Document</h3>
