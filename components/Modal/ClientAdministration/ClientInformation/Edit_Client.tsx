...
import { updateClientAdditional } from '@/services/clientAdministration/clientInformationServices';
...
interface ClientProps {
  ...
  clientID: string;
  refreshList: () => void;
  clientAdditional: ClientInformationClientsProps;
  showDeleteModal: () => void;
}

export default function Edit_ClientInformation_Client({ isOpen, closeModal, clientID, refreshList, clientAdditional, showDeleteModal } : ClientProps) {
  const [ showPanel, setShowPanel ] = useState(true);
  const [ loading, setLoading ] = useState<boolean>(false);

  const [ formData, setFormData ] = useState<ClientInformationClientsProps>(clientAdditional);
  ...
  useEffect( () => {
    if(isOpen) {
      ...
      setFormData(clientAdditional)
    }

  }, [isOpen, clientAdditional])
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    await updateClientAdditional({ data: formData, clientID: clientID});
