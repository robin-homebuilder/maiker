...
import { updateProgressClaims } from '@/services/clientAdministration/claimsInvoicesServices';
...
export default function Edit_ProgressClaim({ isOpen, closeModal, refreshList, clientID, progressClaims, showDeleteModal } : ClientProps) {
  ...
  const [ formData, setFormData ] = useState<ProgressClaimsProps>(progressClaims);
  ...
  useEffect( () => {
    if(isOpen) {
      ...
      const defaultValue = ClaimsInvoiceStatusOptions.find(option => option.value === progressClaims.status);

      setSelectValue(defaultValue);

      const date = convertDateFormat(progressClaims.document_date!);
      setSelectedDate(date || "");

      setFormData({
        _id: progressClaims._id,
        name: progressClaims.name,
        claim_amount: progressClaims.claim_amount,
        status: progressClaims.status,
        file_name: progressClaims.file_name,
        document_date: new Date(progressClaims.document_date!)
      })
    }

  }, [isOpen, progressClaims])
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    await updateProgressClaims({ data: formData, clientID: clientID });
          ...
          <div className='flex min-h-full items-center justify-center text-center top-[-100vw] static w-full h-full'>
            ...
            >
              <Dialog.Panel className='relative w-[525px] h-auto transform rounded-[20px] bg-white text-left shadow-xl transition-all text-dark border border-[#7D7D7D] p-[30px]'>
                ...
                <h3 className="text-[#1C7FCD] text-[25px] font-[800] mb-[25px]">Edit Progress Claim</h3>
