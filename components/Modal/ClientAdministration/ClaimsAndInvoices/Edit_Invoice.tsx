...
import { updateInvoice } from '@/services/clientAdministration/claimsInvoicesServices';
...
export default function Edit_Invoice({ isOpen, closeModal, refreshList, clientID, invoiceData, showDeleteModal } : ClientProps) {
  ...
  const [ formData, setFormData ] = useState<InvoiceProps>(invoiceData);
  ...
  useEffect( () => {
    if(isOpen) {
      ...
      const defaultValue = ClaimsInvoiceStatusOptions.find(option => option.value === invoiceData.status);

      setSelectValue(defaultValue);

      const date = convertDateFormat(invoiceData.document_date!);
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    await updateInvoice({ data: formData, clientID: clientID });
          ...
          <div className='flex min-h-full items-center justify-center text-center top-[-100vw] static w-full h-full'>
            ...
            >
              <Dialog.Panel className='relative w-[525px] h-auto transform rounded-[20px] bg-white text-left shadow-xl transition-all text-dark border border-[#7D7D7D] p-[30px]'>
                ...
                <h3 className="text-[#1C7FCD] text-[25px] font-[800] mb-[25px]">Edit Invoice</h3>
