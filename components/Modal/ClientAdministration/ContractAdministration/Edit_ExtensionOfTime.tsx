...
import { updateExtensionTime } from '@/services/clientAdministration/contractAdministrationServices';
...
interface ClientProps {
  ...
  refreshList: () => void;
  clientID: string,
  clientDocument: ExtensionTimeProps,
  showDeleteModal: () => void;
}

export default function Edit_ExtensionOfTime({ isOpen, closeModal, refreshList, clientID, clientDocument, showDeleteModal } : ClientProps) {
  ...
  const [ formData, setFormData ] = useState<ExtensionTimeProps>(clientDocument);
  ...
  useEffect( () => {
    if(isOpen) {
      ...
      const defaultValue = ClaimsInvoiceStatusOptions.find(option => option.value === clientDocument.status);

      setSelectValue(defaultValue);

      const date = convertDateFormat(clientDocument.document_date!);
      setSelectedDate(date || "");

      setFormData({
        _id: clientDocument._id,
        name: clientDocument.name,
        days_submitted: clientDocument.days_submitted,
        status: clientDocument.status,
        file_name: clientDocument.file_name,
        document_date: new Date(clientDocument.document_date!)
      })
    }

  }, [isOpen, clientDocument])
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    await updateExtensionTime({ data: formData, clientID: clientID });
          ...
          <div className='flex min-h-full items-center justify-center text-center top-[-100vw] static w-full h-full'>
            ...
            >
              <Dialog.Panel className='relative w-[525px] h-auto transform rounded-[20px] bg-white text-left shadow-xl transition-all text-dark border border-[#7D7D7D] p-[30px]'>
                ...
                <h3 className="text-[#1C7FCD] text-[25px] font-[800] mb-[25px]">Add Extension of Time</h3>
                <form className="flex flex-wrap gap-y-[25px]" autoComplete="off" onSubmit={onSubmit}>
                  ...
                  <div className="w-full relative">
                    <input 
                      type="number" 
                      name="days_submitted" 
                      placeholder="Days Submitted / Approved"
                      className="border border-tertiary rounded-[20px] h-[42px] shadow-mainShadow w-full no-spinners"
                      value={formData.days_submitted}
                      onChange={handleInputChange}
                      required
                    />
                  </div>
