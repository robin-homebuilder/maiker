...
import { updateVariation } from '@/services/clientAdministration/contractAdministrationServices';
...
interface ClientProps {
  ...
  refreshList: () => void;
  clientID: string,
  clientDocument: VariationProps,
  showDeleteModal: () => void;
}

export default function Edit_Variation({ isOpen, closeModal, refreshList, clientID, clientDocument, showDeleteModal } : ClientProps) {
  ...
  const [ formData, setFormData ] = useState<VariationProps>(clientDocument);
  ...
  useEffect( () => {
    if(isOpen) {
      ...
      setFormData({
        ...
        amount_submitted: clientDocument.amount_submitted,
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    await updateVariation({ data: formData, clientID: clientID });
          ...
          <div className='flex min-h-full items-center justify-center text-center top-[-100vw] static w-full h-full'>
            ...
            >
              <Dialog.Panel className='relative w-[525px] h-auto transform rounded-[20px] bg-white text-left shadow-xl transition-all text-dark border border-[#7D7D7D] p-[30px]'>
                ...
                <h3 className="text-[#1C7FCD] text-[25px] font-[800] mb-[25px]">Edit Variation</h3>
