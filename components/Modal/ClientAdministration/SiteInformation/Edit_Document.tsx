...
import { updateDocument } from '@/services/clientAdministration/siteInformation';
...
interface ClientProps {
  isOpen: boolean;
  closeModal: () => void;
  clientDocument: SiteDocumentProps,
  clientID: string,
  refreshList: () => void;
  showDeleteModal: () => void;
}

export default function Edit_SiteInformation_Document({ isOpen, closeModal, clientDocument, clientID, refreshList, showDeleteModal } : ClientProps) {
  ...
  const [ formData, setFormData ] = useState<SiteDocumentProps>({
    _id: "",
    name: "",
    file_name: "",
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    await updateDocument({ data: formData, id: clientID })
          ...
          <div className='flex min-h-full items-center justify-center text-center top-[-100vw] static w-full h-full'>
            ...
            >
              <Dialog.Panel className='relative w-[525px] h-auto transform rounded-[20px] bg-white text-left shadow-xl transition-all text-dark border border-[#7D7D7D] p-[30px]'>
                ...
                <h3 className="text-[#1C7FCD] text-[25px] font-[800] mb-[25px]">Edit Document</h3>
