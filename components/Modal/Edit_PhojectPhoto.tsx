...
import { UpdateCompletedProject, createCompletedProject } from "@/services/administration/projectServices";
...
interface ProjectImageURLProps {
  ...
  photoID: string,
  otherImages: string[],
  otherImagesList: string[],
  mainImageName: string
  refreshList: () => void;
}

export default function Edit_ProjectPhoto({ isOpen, closeModal, mainImage, photoID, otherImages, otherImagesList, mainImageName, refreshList } : ProjectImageURLProps) {
  const [ showPanel, setShowPanel ] = useState(true);

  const [ imagePreviewUrl, setImagePreviewUrl ] = useState<string>("");
  const [ imageName, setImageName ] = useState<string>("");

  const [ otherImageList, setOtherImageList ] = useState<string[]>([]);
  ...
  const [ isRequiredMain, setIsRequiredMain ] = useState<boolean>(false);
  const [ isRequiredOther, setIsRequiredOther ] = useState<boolean>(false);
  ...
  useEffect( () => {
    ...
    setImageName(mainImageName);
    setOtherImagePreviewUrl(otherImages)
    setOtherImageList(otherImagesList);
  }, [mainImage, otherImages, otherImagesList, mainImageName])
  ...
  useEffect( () => {
    if(isOpen) {
      ...
      setIsRequiredMain(false);
      setIsRequiredOther(false);
    }
  ...
  const handleFileOtherImage = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const fileNamesArray = newFilesArray.map(file => file.name);
    setOtherImageList((prevImage) => [...prevImage, ...fileNamesArray]);
  ...
  const handleRemoveOtherImage = (index: number) => {
    ...
    setOtherImageList((prevImage) => {
      const updateImageList = [...prevImage];
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    await UpdateCompletedProject({ data: formData, id: photoID, requiredMain: isRequiredMain, requiredOther: isRequiredOther, otherImageList: otherImageList });
          ...
          <div className='flex min-h-full items-center justify-center text-center top-[-100vw] static w-full h-full'>
            ...
            >
              <Dialog.Panel className='relative w-[1000px] h-auto min-h-[550px] overflow-y-auto transform rounded-[20px] bg-white text-left shadow-xl transition-all text-dark border border-[#7D7D7D] p-[30px]'>
                <h3 className="text-[#1C7FCD] text-[25px] font-[800] mb-[25px]">Edit Photo</h3>
